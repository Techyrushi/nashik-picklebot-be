...
router.get("/availability", auth, async (req, res) => {
  try {
    ...
    const bookings = shouldCheckBookings
      ? await Booking.find({
          date: date,
          slotId: slotId,
    ...
    const bookedCourtIds = bookings.map((booking) =>
    ...
    const courtsAvailability = courts.map((court) => {
      ...
      const courtBookings = bookings.filter(
        (b) => b.courtId.toString() === court._id.toString()
      );
    ...
    const availableCourts = courtsAvailability.filter(
      (court) => !court.isFullyBooked
    );
...
router.get("/slots/availability", auth, async (req, res) => {
  try {
    ...
    const totalCapacity = courts.reduce(
      (sum, court) => sum + (court.capacity || 2),
      0
    );
    ...
    const courtsAvailability = courts.map((court) => {
      ...
      return {
        ...
        bookedCapacity: courtBookings.length,
        availableCapacity:
          courtAvailableCapacity > 0 ? courtAvailableCapacity : 0,
        isAvailable: !isFullyBooked,
        isFullyBooked: isFullyBooked,
      };
    });
